import{a as d}from"./api-6da0aff2.js";let l="";const u=document.querySelector(".header-form"),m=document.querySelector(".header"),o=document.querySelector(".header-find");document.querySelector(".header-output");document.querySelector(".header-icon");document.querySelector(".header-btn");function p(e){const r=e.map(t=>{const{id:s,thumbnail:i,name:h,description:C}=t;return`
    ...
    `}).join("");o.insertAdjacentHTML("beforeend",r)}const f=async e=>{e.preventDefault();const{target:r}=e;if(l=r.elements.searchQuery.value,u.reset(),l.trim()==""){console.log("Please specify your search query.");return}const t=await d.getCharacters({nameStartsWith:l});if(t.lengt==0){console.log("Search result is zero. Change your query"),o.innerHTML="";return}else o.innerHTML="",p(t)};u.addEventListener("submit",f);window.addEventListener("scroll",()=>{!window.scrollY==0?m.classList.add("header-color"):m.classList.remove("header-color")});const g=document.querySelector(".rc-list"),y=document.querySelector(".rc-descr-list");let c=0,a=null,n=null;const $=()=>Math.round(Math.random()*1561),v=async()=>{let e=[];for(let r=0;r<5;r+=1){const t=await d.getCharacters({limit:1,offset:$()});if(t[0].thumbnail.path==="http://i.annihil.us/u/prod/marvel/i/mg/b/40/image_not_available"||!t[0].description){r-=1;continue}else t[0].thumbnail.path.includes("https")||t[0].thumbnail.path.replace("http","https");e.push(res)}g.innerHTML=L(e),y.innerHTML=S(e),a=document.querySelectorAll(".rc-list .slide"),n=document.querySelectorAll(".rc-descr-item"),a[0].className="slide rc-item showing",n[0].className="rc-descr-item rc-descr-active",console.log(a[0].className),setInterval(q,3500)};function L(e){return e.map(({id:t,thumbnail:s})=>`  <li class='rc-item slide' data-id="${t}">
      <picture>
